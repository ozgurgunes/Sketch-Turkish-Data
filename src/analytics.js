...
export default function(action, label, value) {
  ...
  if (analyticsAllowed != true) {
    ...
    dialog.setInformativeText(
      'Please allow ' +
        context.plugin.name() +
        ' ' +
        'plugin to use Google Analytics for tracking data.'
    )
  ...
  if (analyticsAllowed) {
    return analytics(action, label, value)
  }
}
...
function analytics(action, label, value) {
  ...
  let payload = {
    ...
    ea: action || context.command.name()
  }
  if (label) {
    payload.el = label
  }
